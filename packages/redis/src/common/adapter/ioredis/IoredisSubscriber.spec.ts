...
describe(IoredisSubscriber.name, () => {
  ...
  beforeAll(() => {
    ...
    ioredisSubscriber = new IoredisSubscriberMock(redisClientMock, messageFromChannelHandler);
  });
  ...
  it('must call IORedis client .on()', () => {
    ...
    expect(redisClientMock.on).toHaveBeenCalledWith('message', expect.any(Function));
  });
